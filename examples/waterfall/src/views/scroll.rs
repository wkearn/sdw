...
pub struct ScrollOverlay<V: View> {
    child: V,
    width: f64,
    background_color: Color,
    foreground_color: Color,
...
impl<V: View> View for ScrollOverlay<V> {
    fn layout(&mut self, min_size: &Size, max_size: &Size) -> Size {
        let child_size = self.child.layout(min_size, max_size);
        ...
        self.actual_size = size;
        self.scroll_position = (child_size.width - self.width).max(0.0);
